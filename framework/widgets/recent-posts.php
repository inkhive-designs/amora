...
class amora_Recent_Posts extends WP_Widget {

	public function __construct() {
		parent::__construct(
			'amora_rp', // Base ID
			__('Amora Recent Posts with Thumbnails', 'amora'), // Name
			array( 'description' => __( 'Display your recent posts, with a Thumbnail.', 'amora' ), ) // Args
		);
	}
	...
	if (isset($instance['title'])) :
		...
		<?php
         ...
         ?>
		         ...
		         <div class='rp-date'><?php the_time(__('M j, Y','amora')); ?></div>
		         </li>      
		...
		?>
		      ...
		      <?php _e('Oops, there are no posts.','amora'); ?>
 	...
 	public function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = __( 'Latest Articles', 'amora' );
		}
		if ( isset( $instance[ 'no_of_posts' ] ) ) {
			$no_of_posts = $instance[ 'no_of_posts' ];
		}
		else {
			$no_of_posts = __( '5', 'amora' );
		}
		?>
		<p>
		<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:','amora' ); ?></label> 
		<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
		
		<label for="<?php echo $this->get_field_id( 'no_of_posts' ); ?>"><?php _e( 'No. of Posts:', 'amora' ); ?></label>
...
add_action( 'widgets_init', 'amora_register_widget' );  
function amora_register_widget() {  
    register_widget( 'amora_Recent_Posts' );  
}  
